  ...
  return (
    <Section isHere={isHere}>
      <InfoSection>
        <ProfileImg>
          {!profileImgSrc ?
            ...
            :
            <img className="profile-image" src={profileImgSrc} alt="profile image" />
          }
        </ProfileImg>
        <TextBox>
      ...
      {!isSelf ? undefined : (
        <ButtonSection>
          <Button>
            <IoMdSettings className='icon' onClick={()=>{navigate(routes.setting)}}/>
...
const Section = styled.div`
  ...
  background-color: ${props =>!props.isHere ? '#FFF3DB' : props.theme.white};
  border-radius: 10px;
